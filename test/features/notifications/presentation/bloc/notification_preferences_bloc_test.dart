...
class MockUpdatePrefs extends Mock
...
void main() {
  ...
  setUp(() {
    ...
    when(() => mockGet()).thenAnswer((_) async =>
        NotificationsResult.success(NotificationPreferencesEntity.defaults()));
    when(() => mockUpdate(any()))
        .thenAnswer((_) async => const NotificationsResult.success(null));

    bloc = NotificationPreferencesBloc(
        getPreferences: mockGet, updatePreferences: mockUpdate);
  });
  ...
  blocTest<NotificationPreferencesBloc, NotificationPreferencesState>(
    ...
    act: (b) =>
        b.add(SavePreferences(NotificationPreferencesEntity.defaults())),
    expect: () => [
      NotificationPreferencesState.initial()
          .copyWith(isSaving: true, clearError: true),
      isA<NotificationPreferencesState>()
  ...
  blocTest<NotificationPreferencesBloc, NotificationPreferencesState>(
    ...
    build: () {
      when(() => mockGet()).thenAnswer((_) async =>
          const NotificationsResult.failure(ServerFailure('fail')));
      return NotificationPreferencesBloc(
          getPreferences: mockGet, updatePreferences: mockUpdate);
    },
    act: (b) => b.add(const LoadPreferences()),
