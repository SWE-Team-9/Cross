...
class GenrePage extends StatelessWidget {
  ...
  final String? genreImagePath; // ← أضف دي
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      ...
      child: _GenreView(
        ...
        genreImagePath: genreImagePath, // ← مررها
      ),
    );
  }
}
      ...
      body: NestedScrollView(
        headerSliverBuilder: (context, innerBoxIsScrolled) => [
          _GenreAppBar(
            ...
            localImagePath: widget.genreImagePath,
          ),
...
class _GenreAppBar extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return SliverAppBar(
      expandedHeight: 280, // ← كبّرنا من 200
      pinned: true,
      backgroundColor: Colors.black,
      collapsedHeight: 120,
      ...
      flexibleSpace: FlexibleSpaceBar(
        collapseMode: CollapseMode.pin, // ← غير من parallax لـ pin عشان متتخفيش
        background: BlocBuilder<GenreCubit, GenreState>(
          builder: (context, state) {
            // نحدد الـ widget بناءً على الأولوية
            Widget imageWidget;
            ...
            if (localImagePath != null) {
              ...
              imageWidget = Image.asset(
                ...
                errorBuilder: (_, __, ___) {
                  // لو فشلت → أولوية 2: API
                  if (state.headerImageUrl.isNotEmpty) {
                    return Image.network(
                      state.headerImageUrl,
                      fit: BoxFit.cover,
                      errorBuilder: (_, __, ___) =>
                          // أولوية 3: color fallback
                          _ColorHeader(color: color, label: label),
                    );
                  }
                  // أولوية 3: color fallback
                  return _ColorHeader(color: color, label: label);
                },
              );
            } else if (state.headerImageUrl.isNotEmpty) {
              // أولوية 2: API
              ...
              // أولوية 3: color fallback
              imageWidget = _ColorHeader(color: color, label: label);
            }
