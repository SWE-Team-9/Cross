...
class RecentPlaylistsStore {
  static const _key = 'recent_playlists_local';

  const RecentPlaylistsStore();

  Future<List<PlaylistEntity>> load({int limit = 10}) async {
    try {
  ...
  Future<void> record(PlaylistEntity playlist, {int limit = 20}) async {
    try {
      ...
      await prefs.setString(
        _key,
        jsonEncode(
          updated.take(limit).map(_playlistToJson).toList(growable: false),
        ),
      );
    } catch (_) {
